...
// Proxies all Notion API requests server-side so the key is never exposed
        ...
        payload.children = [
          {
            ...
            type: 'paragraph',
            paragraph: {
              rich_text: [{ type: 'text', text: { content: body.description } }],
            },
          },
        ];
      }
...
function buildProperties(body) {
  ...
  if (body.location !== undefined)
    props.Location = { rich_text: [{ text: { content: body.location || '' } }] };
  if (body.status !== undefined)
    props.Status = { select: { name: body.status } };
  if (body.type !== undefined)
    props.Type = body.type ? { select: { name: body.type } } : { select: null };
  if (body.people !== undefined)
    props.People = { rich_text: [{ text: { content: body.people || '' } }] };
  if (body.imageUrl !== undefined)
    props['Image URL'] = { url: body.imageUrl || null };
  if (body.sourceUrl !== undefined)
    props['Source URL'] = { url: body.sourceUrl || null };
  if (body.venueUrl !== undefined)
    props['Venue URL'] = { url: body.venueUrl || null };
